...
export default function AnimeCard({
  ...
  const handleToggleFriendsScores = async (e) => {
    e?.stopPropagation?.();
    const nextState = !showFriendsScores;
    setShowFriendsScores(nextState);

    if (nextState) {
      if (Array.isArray(anime.friendsRatings) && anime.friendsRatings.length > 0 && !fetchedRatings) {
        setFetchedRatings(anime.friendsRatings);
      }
      try {
        ...
        console.warn('Failed to load anime ratings:', err);
      } finally {
        setScoresFetchLoading(false);
      }
    }
  };
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        ...
        <div className="mt-5 pt-4 bg-neutral-50/70 dark:bg-neutral-900/50 rounded-2xl p-3.5 sm:p-4">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              {effectiveAvgScore !== null && effectiveRatingCount > 0 ? (
                <button
                  ...
                  title="Посмотреть оценки пользователей сайта"
          ...
          {/* Friends & Community Scores Details */}
          {showFriendsScores && (
            <div className="mt-3 pt-3 border-t border-neutral-200/50 dark:border-neutral-800/60 text-xs animate-in fade-in duration-150">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">
                  Оценки пользователей сайта ({friendsRatings.length}):
                </p>
                {scoresFetchLoading && (
              ...
              {friendsRatings.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {friendsRatings.map((f, idx) => {
                    const isMe = user && (Number(user.id) === Number(f.userId) || user.nickname === f.nickname || f.isMe);
                    return (
                      <div
                        key={f.userId || idx}
                        className={`px-3 py-1.5 rounded-xl flex items-center gap-2 shadow-xs border transition-all ${
                          isMe
                            ? 'bg-amber-500/10 border-amber-500/30 text-amber-900 dark:text-amber-200'
                            : 'bg-white dark:bg-neutral-800 border-neutral-200/60 dark:border-neutral-700/60 text-neutral-800 dark:text-neutral-200'
                        }`}
                      >
                ...
                <div className="py-2.5 text-center text-xs text-neutral-400 flex items-center justify-center gap-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-amber-500" />
                  <span>Загрузка оценок пользователей...</span>
                </div>
              ) : (
                <div className="text-xs text-neutral-400 italic">
                  Пока никто не оценил этот тайтл
                </div>
              )}
            </div>
          )}
